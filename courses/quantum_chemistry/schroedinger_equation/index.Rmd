...
\begin{aligned}
  \psi_1(x) = Ae^{ikx},
\end{aligned}
sendo A uma constante arbitrária, Substituindo $\psi(x)$ teremos
\begin{aligned}
  \frac{\hbar^2}{2m}\frac{d^2}{dx^2}\left(Ae^{ikx}\right) +E\left(Ae^{ikx}\right) & = 0,
\end{aligned}
]
...
\begin{aligned}
  \frac{\hbar^2}{2m}\frac{d^2}{dx^2}\left(Ae^{ikx}\right) +E\left(Ae^{ikx}\right) & = 0,\\
  \frac{\hbar^2}{2m}\left(-Ak^2e^{ikx}\right) +E\left(Ae^{ikx}\right) & = 0,\\
  \frac{-A\hbar^2k^2}{2m}e^{ikx} +AEe^{ikx} & = 0.
\end{aligned}
Podemos ver que $\psi_1(x)$ faz parte da solução se
\begin{aligned}
  E = \frac{\hbar^2k^2}{2m}.
\end{aligned}
Porém, podemos ver que $\psi_2(x)=Be^{-kx}$ também pode ser outra solução para o problema,
\begin{aligned}
  \frac{\hbar^2}{2m}\frac{d^2}{dx^2}\left(B'e^{-ikx}\right) +E\left(B'e^{-ikx}\right) & = 0,\\
  \frac{\hbar^2}{2m}\left(-B'k^2e^{-ikx}\right) +E\left(B'e^{-ikx}\right) & = 0,\\
  \frac{-B'\hbar^2k^2}{2m}e^{-ikx} +B'Ee^{-ikx} & = 0,
\end{aligned}
]
.pull-right[
...
\begin{aligned}
  \psi(x) & = A'\cos(kx)+A'i\sin(kx)\\
  & +B'\cos(-kx)+B'i\sin(-kx),\\
  \psi(x) & = (A'+B')\cos(kx)+(A'-B')i\sin(kx).
\end{aligned}
Sabendo que A' e B' são constantes, então podemos substituí-las por outras constantes, onde $A=A'+B'$ e $B=(A'-B')i$, assim
\begin{aligned}
  \psi(x) & = A\cos(kx)+B\sin(kx).
\end{aligned}
A e B são chamadas constantes de normalização.
]
