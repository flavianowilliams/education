...
\begin{equation}
  \vec{Q} = \vec{q}_1+\vec{q}_2+\vec{q}_3+\therefore \vec{Q} = \sum\vec{q}.
\end{equation}
...
Portanto, podemos dizer que somente haverá variação na quantidade de movimento total de um sistema se houver forças externas. A figura ao lado mostra um sistema isolado formado por cinco partículas de cor azul. Perceba que as bolas pretas não interagem com as bolas azuis.
]
.pull-right[

```{r, echo=FALSE, fig.align='center', fig.cap='Representação de um sistema isolado.', out.width='85%'}
knitr::include_graphics("img/sistema-isolado.jpeg")
```

Com isso chegamos ao enunciado da lei da conservação da quantidade de movimento `r Citep(bib, "Alvarenga2000")``:
