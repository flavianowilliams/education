...
<div class="my-header"><span>A equação de Schroedinger - Considerações iniciais</span></div>
...
Portanto, para estudar o sistema físico, o primeiro passo seria construir essa equação diferencial. Agora, qual seria a variável de interesse, que pode ser obtida a partir dessa equação diferencial? Bem... como foi mencionado na aula sobre o princípio da incerteza de Heisenberg, um elétron pode ser estudado do ponto de vista quântico a partir da função de onda que descreve o seu movimento. Portanto, a solução desejada após resolver essas equação diferencial deverá ser a função de onda do sistema físico.
]
.pull-right[
...
\begin{aligned}
  E = \frac{p^2}{2m}+V;
\end{aligned}
3. Ela deve ser linearmente dependente, onde os seus termos são funções do tipo $\psi(x,t)$;
...
<div class="my-header"><span>A equação de Schroedinger - Representação matemática</span></div>
...
\end{aligned}
Podemos ver que as equações coincidem se $\alpha=\hbar^2k^2/2m$ e $\beta=i\hbar$. Assim chegamos na **tão desejada** equação de Schroedinger em uma dimensão,
\begin{aligned}
  \boxed{-\frac{\hbar^2}{2m}\frac{\partial^2}{\partial x^2}\psi(x,t)+V(x,t)\psi(x,t) = \beta\frac{\partial}{\partial t}\psi(x,t).}
\end{aligned}
]
