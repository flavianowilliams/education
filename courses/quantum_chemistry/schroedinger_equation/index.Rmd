...
\psi(x)=
  ...
  \end{cases}
\end{aligned}
Podemos ver que a solução ainda não está completa, pois falta os valores das constantes $c_1$, $c_2$ e $c_3$. Como poderemos obtê-las? Uma maneira seri empregar a condição de que a função de onda e sua derivada devem ser contínuas ao longo de todo o valor de x, ou seja,
\begin{aligned}
...
\begin{aligned}
  c_2 = \frac{c_3(ik_1+\kappa)}{2ik_1}.
\end{aligned}
Assim, podemos substituir na solução de $\psi(x)$ chegando a
\begin{aligned}
\psi(x)=
  \begin{cases}
    \left(\frac{c_3(ik_1-\kappa)}{2ik_1}\right)e^{ik_1x}+\left(\frac{c_3(ik_1+\kappa)}{2ik_1}\right)e^{-ik_1x},\; & (x<0),\\
    c_3e^{-\kappa x},\; & (x\ge 0).
  \end{cases}
\end{aligned}
Podemos perceber que para chegar na solução final devemos conhecer $c_3$. Usaremos como recurso que a probabilidade de encontrar a partícula em qualquer posição ao longo do eixo x é 100%, ou seja,
\begin{aligned}
  \int^{\infty}_{\infty}|\psi(x)|^2dx & = 1.
\end{aligned}
(*Não se preocupe se não estiver claro, pois esse assunto será aborado com mais detalhes futuramente*).]
.pull-right[
...
\psi(x)=
  \begin{cases}
    \frac{(ik_1-\kappa)\sqrt{2\kappa}}{2ik_1}e^{ik_1x}+\frac{(ik_1+\kappa)\sqrt{2\kappa}}{2ik_1}e^{-ik_1x},\; & (x<0),\\
...
\begin{aligned}
  |\psi(x)|^2 & = \left(c_1e^{ik_1x}+c_2e^{-ik_1x}\right)^*\left(c_1e^{ik_1x}+c_2e^{-ik_1x}\right),\\
  |\psi(x)|^2 & = \left(c^\star_1e^{-ik_1x}+c^\star_2e^{ik_1x}\right)\left(c_1e^{ik_1x}+c_2e^{-ik_1x}\right),\\
  & = c^\star_1c_1e^0+c^\star_1c_2e^{-2ik_1x}+c_1c^\star_2e^{2ik_1x}+c^\star_2c_2e^0.
\end{aligned}
Além disso, $c_1$ e $c_2$ já foram obtidas anteriormente. Substituindo os seus valores teremos
\begin{aligned}
  |\psi(x)|^2 & =
\end{aligned}
]
