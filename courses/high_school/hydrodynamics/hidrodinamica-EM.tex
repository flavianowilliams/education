...
\begin{frame}{Pressão, velocidade e altura de um fluido em duas regiões distintas (continuação)}
	\begin{columns}
		...
		\end{column}
		\begin{column}[t]{0.5\textwidth}
			Substituindo $\Delta E_m$ e $\Delta E_p$ chegaremos na equação abaixo, onde podemos perceber que a soma dos termos abaixo é invariante para qualquer região do tubo. A dedução abaixo exige muita dedução algébrica, portanto deixaremos a sua dedução no Apêndice B.
			\begin{equation*}
...
\begin{frame}{Apêndice A. Alfabeto grego}
	\begin{columns}
...
\begin{frame}{Apêndice B. Dedução da equação de Bernoulli}
	\begin{columns}
		\begin{column}[t]{0.45\textwidth}
			Substituindo $\Delta E_c$ e $\Delta E_p$ e separando os termos da região 1 da região 2 temos a equação
			\begin{align*}
