...
elementos_df = elementos_df %>%
  mutate(carga_nuclear = c(1.00, 1.69, 2.69, 3.68, 4.68, 5.67, 6.66, 7.66, 8.65, 9.64, 10.63, 11.61, 12.59, 13.57, 14.58, 15.54, 16.52, 17.51, 3.49, 4.40)) %>%
...
\begin{align*}
  ...
  E_n & = -\frac{n^2}{2Z}.
\end{align*}
Entretanto, Bohr pensou no modelo do átomo de hidrogênio constuído apenas para o sistema de uma particula sujeita a um potencial coulombiano, ou seja, um elétron. No caso de sistema formados por mais de um elétron, que no caso seria quase todos os elementos químicos da tabela periódica exceto hidrogênio, vimos que o princípio da indistinguibilidade impede a descrição completa da interação existente entre elétrons. Isso inviabiliza qualquer modelo teórico exato que possui solução analítica, como por exemplo o modelo de Bohr. Contudo, veremos que com algumas correções o modelo de Bohr pode de certa maneira explicar alguns comportamentos de átomos, como He, Li, etc...
...
fig = ggplot(raio_atomico_df, aes(x = Z, y = Raio, color = Modelo))+
  ...
  annotate("text", x = 7, y = elementos_df$raio_atomico[19]+0.4, label = "Período 1")+
  ...
  annotate("text", x = 15, y = elementos_df$raio_atomico[19]+0.4, label = "Período 2")
...
for (Z in 1:2){
  energia_ionizacao_df = data.frame(Z=Z, Teórico = rn(1, Z))
}
for (Z in 3:10){
...
fig = ggplot(energia_ionizacao_df, aes(x = Z, y = Energia, color = Modelo))+
  geom_line()+
  geom_point()#+
#  annotate("segment", x = 3, xend = 11, y = elementos_df$energia_ionizacao[19], yend = elementos_df$energia_ionizacao[19], arrow = arrow(ends = "both", angle = 90, length = unit(.2,"cm")))+
#  annotate("text", x = 7, y = elementos_df$energia_ionizacao[19], label = "Período 1")+
#  annotate("segment", x = 11, xend = 19, y = elementos_df$energia_ionizacao[19], yend = elementos_df$energia_ionizacao[19], arrow = arrow(ends = "both", angle = 90, length = unit(.2,"cm")))+
#  annotate("text", x = 15, y = elementos_df$energia_ionizacao[19], label = "Período 2")
...
Definimos afinidade eletrônica como a capacidade do átomo receber um elétron, ou seja, a diferença de energia do ânion e o átomo neutro qualquer,
\begin{align*}
  A = E_{(-)}-E
\end{align*}
...
for (Z in 1:2){
  afinidade_eletronica_df = data.frame(Z=Z, Teórico = rn(1, Z))
}
for (Z in 3:10){
...
fig = ggplot(afinidade_eletronica_df, aes(x = Z, y = Energia))+
  geom_line()+
  geom_point()#+
#  annotate("segment", x = 3, xend = 11, y = elementos_df$afinidade_eletronica[19]+0.2, yend = elementos_df$afinidade_eletronica[19]+0.2, arrow = arrow(ends = "both", angle = 90, length = unit(.2,"cm")))+
#  annotate("text", x = 7, y = elementos_df$afinidade_eletronica[19]+0.4, label = "Período 1")+
#  annotate("segment", x = 11, xend = 19, y = elementos_df$afinidade_eletronica[19]+0.2, yend = elementos_df$afinidade_eletronica[19]+0.2, arrow = arrow(ends = "both", angle = 90, length = unit(.2,"cm")))+
#  annotate("text", x = 15, y = elementos_df$afinidade_eletronica[19]+0.4, label = "Período 2")
...
<div class="my-header"><span>A tabela periódica. Carga efetiva (EDITAR)</span></div>
