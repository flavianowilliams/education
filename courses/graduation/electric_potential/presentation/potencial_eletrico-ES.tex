...
\begin{frame}{Potencial Produzido por uma Part\'icula Carregada}
	\begin{multicols}{2}
		...
		\begin{equation*}
			\Delta V = -\int_{i}^{f}K\frac{Q}{r^2}\hat{r}\cdot d\vec{r} = -\int_{i}^{f}K\frac{Q}{r^2}dr,
		\end{equation*}
		\begin{equation*}
...
\begin{frame}{Potencial Produzido por um dipolo el\'etrico (continua\c c\~ao)}
	\begin{columns}
		\begin{column}[t]{0.6\textwidth}
			...
			\begin{align*}
				r_--r_+ \approx dcos\theta,\quad r_-r_+ \approx r^2.
			\end{align*}
		...
		\begin{column}[c]{0.35\textwidth}
			\begin{figure}
				\begin{tikzpicture}[scale=0.7,transform shape,font=\Large]
					...
					\node[left] at(0,\x) {\tiny $+$};
					\filldraw (p1) circle (1.5pt) node[right] (n1) at (2,-3) {P};
					\draw[dotted] (p1) -- (p2) node [midway,above] {$d$};
					\draw[dotted] (O) -- (p1) node [midway,above right] {$r$};
					\draw ($(O)+(0,0.2)$) to [short dim arrow'={label'=$dz$}] ($(O)+(0,-0.2)$);
					\draw[line width=3pt,color=red] (0,-0.2) --++ (0,0.4);
				\end{tikzpicture}
...
%		\begin{column}[t]{0.5\textwidth}
			\begin{equation*}
				V = \frac{\lambda}{4\pi\varepsilon_0}\left. \ln{\left[z+(z^2+d^2)^{1/2}\right]}\right|_{0}^{L}.
			\end{equation*}
			Usando a identidade $\ln{A}-\ln{B}=\ln{\left(A/B\right)}$ chegamos ao resultado final,
			\begin{equation*}
