...
potencial = function(de, beta, x, x0){
  valor = 0.5*de*beta**2*(x-x0)**2
  return(valor)
}

x_max = function(e, de, beta, x0){
  valor = x0+sqrt(e/de)/beta
}
...
beta = 3.5
x0 = 2.5
x=seq(0,xm,2*xm/smpl)
#

df2 = data.frame(x=x, fr=1, Harmônico = harmonico(de, beta, 1), Morse = morse(de, beta, 1))

df2 = df2 %>%
  gather("Harmônico", "Morse", key = "Potencial", value = "y") %>%
  filter(
      x >= -x_max(harmonico(de, beta, 1), de, beta, -x0) & x <= x_max(harmonico(de, beta, 1), de, beta, x0)
  )

df2 = rbind(df2,
            data.frame(x=x, fr=1, Potencial="Harm", y = potencial(de, beta, x, x0))
            )

for (i in 2:10) {
    df0 = data.frame(x=x, fr=i, Harmônico = harmonico(de, beta, i), Morse = morse(de, beta, i)) %>%
  gather("Harmônico", "Morse", key = "Potencial", value = "y") %>%
  filter(
      x >= -sqrt(2*harmonico(de, beta, i)/de)/beta+x0 & x <= sqrt(2*harmonico(de, beta, i)/de)/beta+x0
  )
df2 = rbind(df2,
            df0,
            data.frame(x=x, fr=i, Potencial="Harm", y = potencial(de, beta, x, x0))
            )
}
...
#  gather("Harmônico", "Morse", key = "Potencial", value = "y") %>%
#  filter(
#      x > -sqrt(2*harmonico(1, de, beta, 1)/de)/beta+x0 & x < sqrt(2*harmonico(1, de, beta, 1)/de)/beta+x0
#  )
...
#
df = data.frame(x=x, n=1, Harmônico = harmonico(0.25, de, beta, x, x0), Morse = morse(0.25, de, beta, x, x0))

for (i in 1:10) {
  df = rbind(df, data.frame(x=x, n=i, Harmônico = harmonico(0.25*i, de, beta, x, x0), Morse = morse(0.25*i, de, beta, x, x0)))
}
...
p = ggplot(data=df, aes(x=x, y=y, frame=n, colour=Potencial))+
  geom_line()+
  ylim(-7, 4)+
